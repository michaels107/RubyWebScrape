...
**_IMPORTANT: If you already had a particular manager role in a previous project then do not take on that manager role again in this assignment._**
...
* Accomplish any tutorials and related reading material indicated in the _[Resources](http://web.cse.ohio-state.edu/~shareef.1/3901.au18/resources/)_ section of the course website
...
* Use your team's private repository on GitHub to help manage the development of your team's solution. We will provide this private repository for your team in our GitHub organization called cse3901-osu-2018au.
...
* [Careers at Ohio State](http://www.jobsatosu.com/) (See "Search Jobs")
* [OSU News Room](http://news.osu.edu/news/)
* Request a [meeting](http://president.osu.edu/contact/meeting-request.html) with the university president
* [Courses](http://registrar.osu.edu/courses/index.asp) as listed by the registrar

For example, your program might search the jobsatosu site for all job postings related to HTML or CSS. You could even use a cron job (see cron and crontab) to run the script, so that every morning your script queries for particular job postings, creates a digest, and sends you an email with the results.
...
You can use whatever source for data you like, provided it is an OSU source and is publicly visible. If you are doing many successive queries, you must space them out in time (eg using sleep between successive GETs) so as to not overload the server. You can do whatever scraping and processing you like, provided the result is interesting and doesn't violate any department or university policies.

(A gem such as Mechanize can be very helpful for issuing HTTP requests, and a gem such as Nokogiri can help navigate the body of the response to find information of interest. Together, these two gems can greatly simplify your web scraping task. For instructions on how to make use of gems, see this [reference](http://web.cse.ohio-state.edu/~shareef.1/3901.au18/labs/gems.html).)

## Grading

* 78% Implementation
Use this link to view [guidelines](http://web.cse.ohio-state.edu/~shareef.1/3901.au18/labs/GradingGuidelines.pdf) when your group is formulating its solution. Though, this is not
the finalized grading template that the graders will use.
...
provided for you under the course's organization cse3901-osu-2018au. All team member's work must eventually be merged 
